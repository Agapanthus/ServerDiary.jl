...
function getSarDay(command::String, keyword::String, useDate::DateTime)
    ...
    # Slow but simple.
    local data = map(x->convert(Array{Any,1},map(x->strip(x),filter(x->length(x)>0,split(x, " ")))), filter(x->length(x)>0, split(res, "\n")))
...
function collectData(command::String, keyword::String, days::Int, today::DateTime)
    ...
    for day in 1:days-1
        logException(_->begin
            local description2, header2, data2, myCommands2, points2 = getSarDay(command, keyword, today - Dates.Day(day))
        ...
        end, "getting sar data from $day days before")
    end

    # detect row types (label or numerical)
    for i in 2:length(header)
        #@assert header[i] in keys(myCommands) "coudln't find $(header[i]) in $(keys(myCommands))"
        header[i] in keys(myCommands) || logger("", "coudln't find $(header[i]) in $(keys(myCommands))")
    ...
    for i in 1:length(datas)
        ...
        logger("length = $(length(data[2]))\nlabels = $(data[1])\nCMD = $command $keyword\npoints = $(length(points))\ndescription = $description", "Created sar table")
